...
interface AiPurchaseModalProps {
  onClose: () => void;
  onStartTask: (file: File, prompt: string) => void;
}

const AiPurchaseModal: React.FC<AiPurchaseModalProps> = ({ onClose, onStartTask }) => {
  const [file, setFile] = useState<File | null>(null);
  const [prompt, setPrompt] = useState('Bu faturadaki ürünleri, adetlerini ve birim alış fiyatlarını listeye ekle.');
     ...
     <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
        ...
        >
            <header className="p-5 border-b flex justify-between items-center flex-shrink-0 bg-slate-50/80">
                <h2 className="text-2xl font-bold text-slate-800 flex items-center gap-3"><Icon name="ai" className="w-7 h-7 text-pink-500" /> AI ile Fatura Oku</h2>
            ...
            <main className="p-6 flex-grow overflow-y-auto">
                <div className="flex flex-col gap-6">
                    <div>
                        <label className="block text-sm font-medium text-slate-600 mb-2">1. Tedarikçi Faturasını veya Listesini Yükleyin</label>
                        <div 
                            onDrop={handleDrop}
                            ...
                            onClick={() => document.getElementById('ai-purchase-file-input')?.click()}
                        >
                            <input id="ai-purchase-file-input" type="file" className="hidden" onChange={handleFileChange} accept="image/*,application/pdf,.csv,.xlsx,.xls" />
                     ...
                     <div>
                        <label htmlFor="ai-prompt" className="block text-sm font-medium text-slate-600 mb-2">2. Yapay Zekaya Talimat Verin</label>
                        <textarea
                            id="ai-prompt"
                            rows={4}
                            value={prompt}
